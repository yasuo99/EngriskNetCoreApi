...
namespace Application.Services.Core
{
    public class AppService : IAppService
    {
        ...
        public async Task<HomeDTO> GetHomeScreenDataAsync(int accountId)
        {
            ...
            homeDTO.Quizzes = _mapper.Map<List<QuizDTO>>(sharedQuiz.Concat(sysQuiz).GetAmountRandomFromAList(10));
            var topUsers = await _context.Accounts.OrderByDescending(orderBy => orderBy.Exp).Take(10).ToListAsync();
